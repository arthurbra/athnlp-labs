...
class NmtSeq2Seq(Model):
    """
    This ``NmtSeq2Seq`` class is an adaptation from the SimpleSeq2Seq :class:`Model` from the AllenNLP toolkit,
    which takes a sequence, encodes it, and then uses the encoded representations to decode another sequence.
    We have removed some functionality .
    ...
    decoder : ``Dict``, required
        The parameters for the decoder RNN cell of the "encoder/decoder" model
    max_decoding_steps : ``int``
        Maximum length of decoded sequences.
        You can specify an embedding dimensionality for the target side. If not, we'll use the same
        value as the source embedder's.
    attention : ``Dict``, optional (default = None)
        If you want to use attention to get a dynamic summary of the encoder outputs at each step
        of decoding, this is the Dict that holds parameters for the appropriate attention function to compute similarity
        between the decoder hidden state and encoder outputs.
